...
macro_rules! define_plaits_param {
    ($fn_name:ident, $osc_param:expr, $error_cmd:expr, $display_name:expr, $parse_ctx:expr) => {
        pub fn $fn_name<F>(
            parts: &[&str],
            variables: &Variables,
            ...
            F: FnMut(String),
        {
            ...
            metro_tx
                .send(MetroCommand::SendParam($osc_param.to_string(), OscType::Float(scaled_value)))
                .context("Failed to send param to metro thread")?;
            if debug_level >= TIER_CONFIRMS || out_cfm {
                output(format!("SET {} TO {} ({:.3})", $display_name, value, scaled_value));
            }
            Ok(())
        }
    };
...
pub(crate) use define_plaits_param;
