...
fn test_tog_independent_instances() {
    ...
    let result1a = eval_expression(&parts1, 0, &variables, &mut patterns, &mut counters, &scripts, 0, &create_test_scale());
    assert_eq!(result1a.unwrap().0, 1);

    let result2a = eval_expression(&parts2, 0, &variables, &mut patterns, &mut counters, &scripts, 0, &create_test_scale());
    assert_eq!(result2a.unwrap().0, 10);

    let result1b = eval_expression(&parts1, 0, &variables, &mut patterns, &mut counters, &scripts, 0, &create_test_scale());
    assert_eq!(result1b.unwrap().0, 2, "TOG 1 2 should be independent and return 2");

    let result2b = eval_expression(&parts2, 0, &variables, &mut patterns, &mut counters, &scripts, 0, &create_test_scale());
...
fn test_tog_per_script_independence() {
    ...
    let result2 = eval_expression(&parts, 0, &variables, &mut patterns, &mut counters, &scripts, 1, &create_test_scale());
